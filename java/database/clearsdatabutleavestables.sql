...
'Gaius Valerius Catullus : Very little is objectively known of the life of Gaius Valerius Catullus, so we should start with one of his poems:
...
<div><img id="img" src="http://artpaintingartist.org/wp-content/uploads/2013/12/Lesbia-by-John-Reinhard-Weguelin.jpg" alt="Lesbia, 1878 painting by John Reinhard Weguelin inspired by the poems of Catullus"/></div>
...
<td>meal, not without a fair-skinned girl
</td></tr>
<tr>
<th>5
</th>
...
</code></pre></div><h2 id="type-conversion"> Type conversion</h2> <p>When you move a value from one type to another, the compiler becomes interested in what youre doing. It worries about whether or not the operation being performed might result in a loss of data. Each conversion operation is considered in terms of <em>widening</em> (smaller data type to larger data type) and <em>narrowing</em> (larger data type to smaller data type) of value ranges.</p> 
...
</code></pre></div></div> <div class="code-block"><!----></div> <h3 id="logical-operators">Logical operators</h3> <p>The comparison operators are limited in their ability to compare values between two different types. When a boolean expression needs to take two or more considerations into account to evaluate a boolean value—for example, is the number negative AND is it even—then the logical operators are used.</p> <p>The logical operators are AND (<code>&amp;&amp;</code>), OR (<code>||</code>), XOR (<code>^</code>), and NOT (<code>!</code>). These operators work with one or more boolean operands to evaluate a boolean expression and yield <code>true</code> or <code>false.</code></p> <h4 id="not"> NOT <code>!</code></h4> 
...
'<!DOCTYPE html>
 ...
 <p>In <span></span>, interfaces force each component to expose specific public members that can be used in a specific way. Interfaces provide you with a way to say what classes can do and as such are an alternative way to define an <em>is a</em> relationship.</p> <p>The code below shows how an interface is defined.</p> <div class="code-block"><!----></div> <div class="code-block"><!----></div> <p>Notice that in the definition of the interface, theres no implementation for the methods. Theres only a method signature, minus the access modifier. Thats what an interface is: a declaration of one or more public methods.</p> 
...
'Today, we will switch things up and learn how to count to ten in Spanish, in two easy steps!

<div><img id="image" src="https://i.pinimg.com/originals/f1/88/03/f188035b3f1b6077449bfe4d78f03339.jpg" alt="counting from one to ten!" /></div>
...
What’s the Big Deal?Retrieved from Towards Data Science: https://towardsdatascience.com/graph-databases-whats-the-big-deal-ec310b1bc0ed </strong>');
...
<div><img id="img" src="hhttp://beckerexhibits.wustl.edu/rare/images1/Kircher1673_162_large.jpg" alt="Acoustic Network Signaling in Kirchers Phonurgia Nova (1673 Facsimile)"></div>
